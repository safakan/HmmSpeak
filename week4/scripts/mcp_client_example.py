...
async def main(
    ...
    async with client:
        ...
        # Tool 2: run trivia tool to generate a question
        result = await client.call_tool("trivia", {})
        trivia_question = result[0].text
        result_dict["trivia"] = trivia_question

        # Tool 3: run textme tool
        result = await client.call_tool("textme", {"text_content": result_dict["trivia"]})
...
if __name__ == "__main__":
    ...
    # text_content = "Congratz on your first sms message! I'm an AIIIIIII lolz"
    # asyncio.run(main(greet_text, text_content))

    asyncio.run(main(greet_text))